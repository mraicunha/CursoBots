...
const formatarData = data => data ? moment(data).format('DD/MM/YYYY') : ''
...
const exibirTarefa = async (ctx, tarefaId, novaMsg = false) => {
  ...
  const conclusao = tarefa.dt_conclusao ?
    `\n<b>Concluída em:</b> ${formatarData(tarefa.conclusao)}` : ''
...
bot.command('concluidas', async ctx => {
  ...
  ctx.reply(`Estas são as tarefas que você ja concluiu`, botoesAgenda(tarefas))
})
...
bot.action(/excluir (.+)/, async ctx => {
  ...
  ctx.editMessageText('Tarefa Excluida')
})
...
obsScene.on('message', ctx => {
  ctx.reply(`Apenas observações em texto são acitas`)
})
